...
var (
	...
	authorizationHeaderPattern = regexp.MustCompile(`(?i)\b(authorization)\s*:\s*(?:bearer\s+)?[^\s|]+`)
	cookieHeaderPattern        = regexp.MustCompile(`(?i)\b(cookie)\s*:\s*[^\s|]+`)
	setCookieHeaderPattern     = regexp.MustCompile(`(?i)\b(set-cookie)\s*:\s*[^\s|]+`)
